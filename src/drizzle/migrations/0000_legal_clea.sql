...
ALTER TABLE `vendor_order_items` ADD CONSTRAINT `fk_vendor_order_id` FOREIGN KEY (`vendor_order_id`) REFERENCES `vendor_orders`(`vendor_order_id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `time_logs` ADD CONSTRAINT `time_logs_worker_id_workers_worker_id_fk` FOREIGN KEY (`worker_id`) REFERENCES `workers`(`worker_id`) ON DELETE restrict ON UPDATE no action;